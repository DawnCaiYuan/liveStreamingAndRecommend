...
exports.douyu = function(categoryPath) {
    ...
    return new Promise((resolve, reject) => {
        get(url)
            .then(({text}) => {
                let liveJson = [];
                for (let item of JSON.parse(text).data.rl) {
                    liveJson.push({
                        title: item.rn,
                        anchor: item.nn,
                        audienceNumber: item.ol,
                        snapshot: item.rs1,
                        url: 'http://www.douyu.com' + item.url,
            ...
            //     let liveJson = [];
            //     let $ = cheerio.load(text);
            //     $('#listAll .layout-Module-container ul li').each((idx, ele) => {
            //         ele = $(ele);
            //         let audienceText = $(ele.find('.DyListCover-hot').first()).text();
            //         liveJson.push({
            //             title: $(ele.find('.DyListCover-intro')[0]).text(),
            //             anchor: $(ele.find('h2.DyListCover-user')[0]).text(),
            //             audienceNumber: transformAudienceNumber(audienceText),
            //             snapshot: $(ele.find('.DyListCover-pic img')[0]).attr('src'),
            //             url: 'https://www.douyu.com' + $(ele.find('a')[0]).attr('href'),
            //             platformIcon: '/images/icon1.png',
            //         });
            //     });
            //     judgeDataAna(judgeDataAna, '斗鱼tv');
            //     resolve(liveJson);
            // })
            // .catch(reject);
    });
